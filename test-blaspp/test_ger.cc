...
//    int64_t verbose = params.verbose();
...
#if 0
    // test error exits
    assert_throw( tmblas::ger( -1,  n, alphat, At, lda, xt, incx, yt, incy ), blas::Error );
    assert_throw( tmblas::ger(  m, -1, alphat, At, lda, xt, incx, yt, incy ), blas::Error );

    assert_throw( tmblas::ger(  m,  n, alphat, At, m-1, xt, incx, yt, incy ), blas::Error );

    assert_throw( tmblas::ger(  m,  n, alphat, At, lda, xt, 0,    yt, incy ), blas::Error );
    assert_throw( tmblas::ger(  m,  n, alphat, At, lda, xt, incx, yt, 0    ), blas::Error );
    ...
    tmblas::ger <TX, TY, TA> ( m, n, alphat, xt, incx, yt, incy, At, lda );
    time = get_wtime() - time;
...
//		A[i] = tmblas::type_conv<RA, TA>(At[i]);
    ...
    if (params.check() == 'y') {
        ...
        testsweeper::flush_cache( params.cache() );
        time = get_wtime();
        blas::ger <RX, RY, RA> ( layout, m, n, alphar, xr, incx, yr, incy, Ar, lda );
        time = get_wtime() - time;
