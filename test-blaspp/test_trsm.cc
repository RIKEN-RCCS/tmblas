...
void test_trsm_work( Params& params, bool run )
{
    ...
    int64_t idist = 1;
    int iseed[4] = { 0, 0, 0, 1 };
    tmblas_larnv<blas::real_type<Tbase> >( idist, iseed, size_A, A );
    ...
    Tbasereal xAm((int)Am);
    for (int64_t i = 0; i < Am; ++i) {
      tmblas::mixedp_add<Tbase, Tbase, Tbasereal>(A[ i + i*lda ], A[ i + i*lda ], xAm);
    }

    int64_t info = 0;
    lapack_potrf( uplo2char(uplo), Am, A, lda, &info );
    require( info == 0 );
#if 0
...
#if 0
    // test error exits
    assert_throw( tmblas::trsm( Side(0), uplo,    trans, diag,     m,  n, alphat, At, lda, Bt, ldb ), blas::Error );
    assert_throw( tmblas::trsm( side,    Uplo(0), trans, diag,     m,  n, alphat, At, lda, Bt, ldb ), blas::Error );
    assert_throw( tmblas::trsm( side,    uplo,    Op(0), diag,     m,  n, alphat, At, lda, Bt, ldb ), blas::Error );
    assert_throw( tmblas::trsm( side,    uplo,    trans, Diag(0),  m,  n, alphat, At, lda, Bt, ldb ), blas::Error );
    assert_throw( tmblas::trsm( side,    uplo,    trans, diag,    -1,  n, alphat, At, lda, Bt, ldb ), blas::Error );
    assert_throw( tmblas::trsm( side,    uplo,    trans, diag,     m, -1, alphat, At, lda, Bt, ldb ), blas::Error );

    assert_throw( tmblas::trsm( Side::Left,  uplo,   trans, diag,     m,  n, alphat, At, m-1, Bt, ldb ), blas::Error );
    assert_throw( tmblas::trsm( Side::Right, uplo,   trans, diag,     m,  n, alphat, At, n-1, Bt, ldb ), blas::Error );

    assert_throw( tmblas::trsm( side, uplo, trans, diag,    m,  n, alphat, At, lda, Bt, m-1 ), blas::Error );
    ...
    if (params.check() == 'y') {
        ...
        testsweeper::flush_cache( params.cache() );
        time = get_wtime();
	blas::trsm<RA, RB>( layout, side, uplo, trans, diag,
...
void test_trsm( Params& params, bool run )
{
    switch (params.datatype()) {
	  ...
	  test_trsm_work< float,
			  float, float,
			  float, float >( params, run );
            break;

        case testsweeper::DataType::Double:
	  test_trsm_work< double,
			  double, double,
	  ...
	  test_trsm_work< double,
        ...
        case testsweeper::DataType::SingleComplex:
            test_trsm_work< std::complex<float>,
        ...
        case testsweeper::DataType::DoubleComplex:
            test_trsm_work< std::complex<double>,
