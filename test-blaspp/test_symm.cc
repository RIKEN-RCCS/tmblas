...
void test_symm_work( Params& params, bool run )
{
...
#if 0
    // test error exits
    assert_throw( tmblas::symm( Side(0),  uplo,     m,  n, alphat, At, lda, Bt, ldb, betat, C, ldc ), blas::Error );
    assert_throw( tmblas::symm( side,     Uplo(0),  m,  n, alphat, At, lda, Bt, ldb, betat, C, ldc ), blas::Error );
    assert_throw( tmblas::symm( side,     uplo,    -1,  n, alphat, At, lda, Bt, ldb, betat, C, ldc ), blas::Error );
    assert_throw( tmblas::symm( side,     uplo,     m, -1, alphat, At, lda, Bt, ldb, betat, C, ldc ), blas::Error );

    assert_throw( tmblas::symm( Side::Left,  uplo,     m,  n, alphat, At, m-1, Bt, ldb, betat, C, ldc ), blas::Error );
    assert_throw( tmblas::symm( Side::Right, uplo,     m,  n, alphat, At, n-1, Bt, ldb, betat, C, ldc ), blas::Error );

    assert_throw( tmblas::symm( side, uplo,  m,  n, alphat, A, lda, Bt, m-1, betat, Ct, ldc ), blas::Error );
    assert_throw( tmblas::symm( side, uplo,  m,  n, alphat, A, lda, Bt, ldb, betat, Ct, m-1 ), blas::Error );
    ...
    if (params.ref() == 'y' || params.check() == 'y') {
        ...
        testsweeper::flush_cache( params.cache() );
        time = get_wtime();
	blas::symm<RA, RB, RC>( layout, side, uplo,
	...
	//check_diff<RC, TC>(Cm, Cn, Cr, Ct, ldc, &errormax, &errorl2, uplo);
	if (trunc) {
...
void test_symm( Params& params, bool run )
{
    switch (params.datatype()) {
	  ...
	  test_symm_work< float,
			  float, float, float,
			  float, float, float >( params, run );
            break;

        case testsweeper::DataType::Double:
	  test_symm_work< double,
	  ...
	  test_symm_work< double,
        ...
        case testsweeper::DataType::SingleComplex:
            test_symm_work< std::complex<float>,
        ...
        case testsweeper::DataType::DoubleComplex:
            test_symm_work< std::complex<double>,
