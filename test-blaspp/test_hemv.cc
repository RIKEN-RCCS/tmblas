...
//    int64_t verbose = params.verbose();
...
#if 0
    // test error exits
    assert_throw( tmblas::hemv( Uplo(0),  n, alphat, At, lda, xt, incx, betat, yt, incy ), blas::Error );
    assert_throw( tmblas::hemv( uplo,    -1, alphat, At, lda, xt, incx, betat, yt, incy ), blas::Error );
    assert_throw( tmblas::hemv( uplo,     n, alphat, At, n-1, xt, incx, betat, yt, incy ), blas::Error );
    assert_throw( tmblas::hemv( uplo,     n, alphat, At, lda, xt,    0, betat, yt, incy ), blas::Error );
    assert_throw( tmblas::hemv( uplo,     n, alphat, At, lda, xt, incx, betat, yt,    0 ), blas::Error );
    ...
    tmblas::hemv <TA, TX, TY> ( uplo, n, alphat, At, lda, xt, incx, betat, yt, incy );
    time = get_wtime() - time;
...
//		y[i] = tmblas::type_conv<RY, TY>(yt[i]);
    ...
    if (params.check() == 'y') {
        ...
        testsweeper::flush_cache( params.cache() );
        time = get_wtime();
        blas::hemv <RA, RX, RY> ( layout, uplo, n,
