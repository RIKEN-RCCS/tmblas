...
void test_syr_work( Params& params, bool run )
{
    ...
    assert_throw(( tmblas::syr<TA,TX>( Uplo(0),  n, alphat, xt, incx, At, lda )), blas::Error );
    assert_throw(( tmblas::syr<TA,TX>( uplo,    -1, alphat, xt, incx, At, lda )), blas::Error );
    assert_throw(( tmblas::syr<TA,TX>( uplo,     n, alphat, xt,    0, At, lda )), blas::Error );
    assert_throw(( tmblas::syr<TA,TX>( uplo,     n, alphat, xt, incx, At, n-1 )), blas::Error );
	#endif
    ...
    tmblas::syr<TA,TX>( uplo, n, alphat, xt, incx, At, lda );
    time = get_wtime() - time;
...
void test_syr( Params& params, bool run )
{
    switch (params.datatype()) {
        case testsweeper::DataType::Single:
            test_syr_work< float, float, float, float >( params, run );
            break;

        case testsweeper::DataType::Double:
            test_syr_work< double, double, double, double >( params, run );
            break;

        case testsweeper::DataType::Quadruple:
            test_syr_work< double, double, quadruple, quadruple >( params, run );
            break;
