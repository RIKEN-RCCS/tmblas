...
void test_syrk_work( Params& params, bool run )
{
    ...
    assert_throw(( tmblas::syrk<TA,TC>( Uplo(0), trans,  n,  k, alphat, At, lda, betat, Ct, ldc )), blas::Error );
    assert_throw(( tmblas::syrk<TA,TC>( uplo,    Op(0),  n,  k, alphat, At, lda, betat, Ct, ldc )), blas::Error );
    assert_throw(( tmblas::syrk<TA,TC>( uplo,    trans, -1,  k, alphat, At, lda, betat, Ct, ldc )), blas::Error );
    assert_throw(( tmblas::syrk<TA,TC>( uplo,    trans,  n, -1, alphat, At, lda, betat, Ct, ldc )), blas::Error );

    assert_throw(( tmblas::syrk<TA,TC>( uplo, Op::NoTrans,   n, k, alphat, At, n-1, betat, Ct, ldc )), blas::Error );
    assert_throw(( tmblas::syrk<TA,TC>( uplo, Op::Trans,     n, k, alphat, At, k-1, betat, Ct, ldc )), blas::Error );
    assert_throw(( tmblas::syrk<TA,TC>( uplo, Op::ConjTrans, n, k, alphat, At, k-1, betat, Ct, ldc )), blas::Error );

    assert_throw(( tmblas::syrk<TA,TC>( uplo, Op::NoTrans,   n, k, alphat, At, k-1, betat, Ct, ldc )), blas::Error );
    assert_throw(( tmblas::syrk<TA,TC>( uplo, Op::Trans,     n, k, alphat, At, n-1, betat, Ct, ldc )), blas::Error );
    assert_throw(( tmblas::syrk<TA,TC>( uplo, Op::ConjTrans, n, k, alphat, At, n-1, betat, Ct, ldc )), blas::Error );

    assert_throw(( tmblas::syrk<TA,TC>( uplo,    trans,  n,  k, alphat, At, lda, betat, Ct, n-1 )), blas::Error );

    if (blas::is_complex<scalar_t>::value) {
        // complex syrk doesn't allow ConjTrans, only Trans
        assert_throw(( tmblas::syrk<TA,TC>( uplo, Op::ConjTrans, n, k, alphat, At, lda, betat, Ct, ldc )), blas::Error );
    }
	#endif
...
void test_syrk( Params& params, bool run )
{
    switch (params.datatype()) {
        case testsweeper::DataType::Single:
            test_syrk_work< float, float, float, float >( params, run );
            break;

        case testsweeper::DataType::Double:
            test_syrk_work< double, double, double, double >( params, run );
            break;

        case testsweeper::DataType::Quadruple:
            test_syrk_work< double, double, quadruple, quadruple >( params, run );
            break;

        case testsweeper::DataType::SingleComplex:
            test_syrk_work< std::complex<float>, std::complex<float>, std::complex<float>, std::complex<float> >
                ( params, run );
            break;

        case testsweeper::DataType::DoubleComplex:
            test_syrk_work< std::complex<double>, std::complex<double>, std::complex<double>, std::complex<double> >
                ( params, run );
            break;

        case testsweeper::DataType::QuadrupleComplex:
            test_syrk_work< std::complex<double>, std::complex<double>, std::complex<quadruple>, std::complex<quadruple> >
                ( params, run );
            break;
