...
#ifndef _SYR2_TMPL_HPP
#define _SYR2_TMPL_HPP
  ...
  if (uplo == blas::Uplo::Upper) {
    if (incx == 1 && incy == 1) {
      ...
      for (idx_int j = 0; j < n; ++j) {
        ...
        mixedp_mul<Td, Tscalar, Tb>(tmp1, alpha, y[j]);
        mixedp_mul<Td, Tscalar, Ta>(tmp2, alpha, x[j]);
...
//          A(i, j) += x[i]*tmp1 + y[i]*tmp2;
      ...
      idx_int jy = ky;
      for (idx_int j = 0; j < n; ++j) {
        mixedp_mul<Td, Tscalar, Tb>(tmp1, alpha, y[jy]);
        mixedp_mul<Td, Tscalar, Ta>(tmp2, alpha, x[jx]);
