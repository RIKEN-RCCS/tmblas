...
#ifndef _HEMM_TMPL_HPP
#define _HEMM_TMPL_HPP
...
//  typedef blas::real_type<Tscalar> Tscalarreal;
  ...
  if (mixedp_eq<Tscalar, int>(alpha, 0)) {
    ...
    else if (!mixedp_eq<Tscalar, int>(beta, 1)) {
      Td tmp;
      for (idx_int j = 0; j < n; ++j) {
        for (idx_int i = 0; i < m; ++i) {
  ...
  if (side == blas::Side::Left) {
      ...
      for (idx_int j = 0; j < n; ++j) {
        for (idx_int i = 0; i < m; ++i) {
          ...
          for (idx_int k = 0; k < i; ++k) {
            mixedp_madd<Td, Ta, Tb>(s, conjg<Ta>(A(k, i)), B(k, j)); 
          }
	  mixedp_madd<Td, Tareal, Tb>(s, real<Tareal>(A(i, i)), B(i, j)); 
    ...
    // uplo == Uplo::Upper or uplo == Uplo::General
	  ...
	  mixedp_madd<Td, Tb, Tareal>(s, B(i, j), real<Tareal>(A(j, j)));

          for (idx_int k = j+1 ; k < n ; ++k) {
            mixedp_madd<Td, Tb, Ta>(s, B(i, k), conjg<Ta>(A(j, k)));
          }
          mixedp_madd<Td, Tscalar, Td>(w, alpha, s);
          C(i, j) = type_conv<Tc, Td>(w);
        }
