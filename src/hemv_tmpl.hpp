...
#ifndef _HEMV_TMPL_HPP
#define _HEMV_TMPL_HPP
...
///     the opposite triangle being assumed from Hermitian symmetry.
    ...
    if (incx == 1 && incy == 1) {
      ...
      for (idx_int j = 0; j < n; ++j) {
        ...
        for (idx_int i = 0; i < j; ++i) {
          ...
          mixedp_madd<Td, Ta, Tb>(tmp2, conjg<Ta>(A(i, j)), x[i]);
          //y[i] += tmp1 * A(i, j);
          //tmp2 += A(i, j) * x[i];
        }
        mixedp_madd<Td, Td, Tareal>(ww[j], tmp1, real<Tareal>(A(j, j)));
      ...
      for (idx_int j = 0; j < n; ++j) {
        ...
        for (idx_int i = 0; i < j; ++i) {
          mixedp_madd<Td, Td, Ta>(ww[i], tmp1, A(i, j));
          mixedp_madd<Td, Ta, Tb>(tmp2, conjg<Ta>(A(i, j)), x[ix]);
//          y[iy] += tmp1 * A(i, j);
//          tmp2 += A(i, j) * x[ix];
          ix += incx;
        }
